 ...
 * @author farud
 */
public class RegistarPDF extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ArtistasServlets</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ArtistasServlets at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        // processRequest(request, response); // Esta línea está comentada y no se ejecuta

        String acceso = ""; // Variable para determinar la página de redirección
        String action = request.getParameter("accion"); // Obtener el parámetro "accion" de la solicitud
        
        
        if (action.equalsIgnoreCase("add")) {
            // Si el valor de "accion" es "add"
            acceso = add; // Establecer la variable de acceso a "add" 
        } else if (action.equalsIgnoreCase("Agregar")) {
            // Si el valor de "accion" es "Agregar" (distingue mayúsculas y minúsculas)
            System.out.println("Ingreso a post");

            // Ruta donde se guardarán los archivos PDF
            String path_project = "C:\\Users\\ADSO\\Documents\\GitHub\\admin_gesdoc--v4\\web\\Arc_PDF\\";
            ...
            if (!fileName.toLowerCase().endsWith(".pdf")) {
                ...
                enviado.setEnvTipoDocumental(envTipoDocumental);
                enviado.setEnvObservaciones(envObservaciones);
                enviado.setEnvArchivoPdf(envArchivoPdf);
